...
close(MIR);
if ($species==9606) {
   $ScoreCard='human_pred_plus_beta_and_shift_fix_ovl_len.txt';
}
elsif ($species==10116) {
   $ScoreCard='rno_scorecard.txt';
}
else {
   $ScoreCard='mouse_pred_plus_beta_and_shift_fix_ovl_len.txt';
}
